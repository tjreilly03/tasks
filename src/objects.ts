...
export function toMarkdown(question: Question): string {
    let resultstring = "";
    resultstring += "# " + question.name + "\n" 
    + question.body;
    if(question.type == "multiple_choice_question"){
        let multipleans = "\n- "
        multipleans += question.options.join("\n- ");
        resultstring +=multipleans;
    }
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newQuestion =  {body: oldQuestion.body, type: oldQuestion.type, options:oldQuestion.options, expected: oldQuestion.expected, points: oldQuestion.points
        , name: "Copy of " + oldQuestion.name, published: false, id:id
    };
    return newQuestion;
}
...
export function addOption(question: Question, newOption: string): Question {
    ...
    const newquestions = {...question, options: newoptions};
    return newquestions;
}
...
export function mergeQuestion(
    ...
    const newQuestion =  {body: contentQuestion.body, type: contentQuestion.type, options:contentQuestion.options, expected: contentQuestion.expected, points: points
        , name: name, published: false, id:id
    };
    return newQuestion;
}
